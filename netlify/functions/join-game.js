...
exports.handler = async (event) => {
  ...
  if (Object.keys(game.players).length >= 10) return { statusCode: 400, body: JSON.stringify({ error: "Game is full (max 10 players)" }) };
  ...
  if (game.players[playerKey]) {
    ...
    return { statusCode: 200, headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ playerKey, cards: p.cards || [p.card], drawnNumbers: game.drawnNumbers, winner: game.winner, winners: game.winners || [], gameName: game.gameName || null }) };
  }
  ...
  try {
    ...
    const playerList = Object.fromEntries(Object.entries(game.players).map(([k, p]) => [k, { name: p.name, joinedAt: p.joinedAt, cardCount: (p.cards || [p.card]).length }]));
    await ably.channels.get(`bingo-${gameId}`).publish("player-joined", { playerKey, name: playerName.trim(), players: playerList });
  } catch (_) {}
  return { statusCode: 200, headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ playerKey, cards, drawnNumbers: game.drawnNumbers, winner: game.winner, winners: game.winners || [], gameName: game.gameName || null }) };
};
